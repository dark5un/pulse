...
def render_leaderboard(
    records: list[dict], *, as_json: bool = False, task: str | None = None, top: int = 3
) -> str:
    if task:
        records = [r for r in records if r.get("task_type") == task]
    ranked = rank_traces(records, top=top)
    if as_json:
    ...
    for t in sorted(ranked):
        lines.append(f"== {t} ({len([r for r in records if r.get('task_type') == t])} traces) ==")
        for bucket in ("best", "worst"):
            lines.append(f"  {bucket.upper()}:")
            for e in ranked[t][bucket]:  # type: ignore[literal-required]
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--task", default=None, help="Only show this task type")
    ap.add_argument("--top", type=int, default=3, help="Entries per bucket")
    args = ap.parse_args(argv)
    print(
        render_leaderboard(
            load_corpus_records(args.corpus), as_json=args.json,
            task=args.task, top=args.top,
        )
    )
    return 0
