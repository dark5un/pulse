...
import { aggregate, latestAnalysis, saveAnalysis, saveFeedback } from "./state.js";
...
export default function pulseExtension(pi: ExtensionAPI) {
  ...
  pi.on("session_start", async () => { lastAutomaticKey=""; });
  pi.on("session_tree", async () => { lastAutomaticKey=""; });
}
